    ...
    const requestOptions = {
      ...
      fetch(`http://localhost:3000/films/${id}`, requestOptions)
        ...
        .then((result) => {
            ...
            const capacity = parseInt(result.capacity);
        const ticketSold = parseInt(result.tickets_sold);
        const remainingTickets = capacity - ticketSold;
            title.innerText = result.title;
            runtime.innerText = `${result.runtime} minutes`;
            info.innerText = result.description;
            showtime.innerText = result.showtime;
            ticket.innerHTML =`<span style="margin-top: 10px; display: block;">Remaining Tickets: ${remainingTickets}</span>`;
